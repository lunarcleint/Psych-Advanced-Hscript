...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadBG()
	{
		...
		{
			if (FileSystem.exists(Paths.getPath('images/menubackgrounds/menu_' + assetName + '.png', IMAGE)))
			{
	...
	function reloadWeekThing()
	{
		...
		if (assetName != null && assetName.length > 0)
		{
			if (FileSystem.exists(Paths.getPath('images/storymenu/' + assetName + '.png', IMAGE)))
			{
